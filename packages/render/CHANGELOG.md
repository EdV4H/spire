...
- ca10b0e: `@edv4h/spire-render/headless` を追加した。メインの入口は `SpireMap` を re-export するので、`renderToSVG` しか呼ばないサーバでも React が必要だった。`headless` は React を import する 3 ファイル（`SpireMap` / `Shapes` / SVG バックエンド）を除いた同じ中身で、レンダラの登録も静的 SVG 出力もここから完結する。あわせて `react` を optional peer にした。
