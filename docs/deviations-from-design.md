...
- **構造（`chokeRows`）** — その行のノードを1つに絞る。迂回路が**存在しない**。
  終端の絞り込みと同じゲート機構で、窓の交差として実装している
- **進行（checkpoint ポリシー）** — マップは分岐したまま、指定タイプのノードを
  1つ通るまで先の行を拒否する。迂回路は存在するが**通れない**

前者はマップを見れば分かり、後者は触らないと分からない。前者は生成時に決まり、
後者は後から差し替えられる。playground の「途中で必ず通る（構造）」と
「（進行）」で並べて比べられる。
