...
## 15. 途中の必須ノードを構造で作る（`chokeRows`）

設計書には「始点から終点へ」の構造しかなく、途中に必ず通る場所を作る手段が無い。
これは2つの層で実現できるので、**両方を実装して playground で並べて比べたうえで、
構造のほうを残した**。

採ったほう — **構造（`chokeRows`）**: その行のノードを1つに絞る。ウォークは必ず
全行を通るので、全ルートがそのノードを通り、**迂回路が存在しない**。終端の絞り込みと
同じゲート機構で、各行の窓をゲートの交差として求めている。

捨てたほう — **進行（checkpoint ポリシー）**: マップは分岐したまま、指定タイプの
ノードを1つ通るまで先の行を拒否する。迂回路は存在するが通れない。2つの違いは:

|  | 構造 | 進行 |
|---|---|---|
| 迂回路 | 存在しない | 存在するが通れない |
| いつ決まる | 生成時（マップに焼き付く） | 実行時（ポリシーを外せば消える） |
| 見て分かるか | マップを見れば分かる | 触るまで分からない |
| 既存マップへの後付け | できない | できる |

進行側が要るとなったら、`ProgressionPolicy` はプラグインで足せるので SDK の変更は
要らない — それが捨てても惜しくない理由でもある。
